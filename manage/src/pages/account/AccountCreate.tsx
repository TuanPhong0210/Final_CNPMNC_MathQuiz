...
const AccountCreate = ({ actionsAllowed }: AccountCreateProps) => {
  const { administrators, teachers, students } = useAppSelector(selectAccount);
  ...
  const propsByType: Omit<AccountGeneralFormProps, 'actionsAllowed'> & { backTo: string } =
    type === 'administrators'
      ...
      : type === 'teachers'
      ? {
          account: isEdit ? teachers.find((teacher) => teacher._id === _id) : undefined,
          account_type: 'Teacher',
          backTo: PATH_DASHBOARD.account.teachers,
        }
      : {
          account: isEdit ? students.find((student) => student._id === _id) : undefined,
          account_type: 'Teacher',
          backTo: PATH_DASHBOARD.account.students,
        };
  return (
    <div>
